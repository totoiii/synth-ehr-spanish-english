#!/bin/bash
# =============================================================================
# MEGA PIPELINE 2 — MIMIC (small + large)
# Train GPU:  2 (NVIDIA RTX 6000 Ada, 48 GB)
# Gen GPUs:   1,2 (Blackwell 96GB + Ada 48GB, TP=2)
# Phases:  Finetune → Merge → Generate
# =============================================================================
#
# Usage:
#   nohup bash scripts/finetuning_and_synthesis_2.sh > output/logs/mega_pipeline_2.log 2>&1 &
#
# To resume from a specific phase:
#   START_PHASE=2 bash scripts/finetuning_and_synthesis_2.sh   # skip finetuning
#   START_PHASE=3 bash scripts/finetuning_and_synthesis_2.sh   # skip to generation
...
MERGE_GPU=1                        # Ada GPU for merging
GEN_GPUS="1"                     # Blackwell + Ada for generation (TP=2)
GEN_TP=1                           # Tensor parallel size for generation

export CUDA_VISIBLE_DEVICES=${TRAIN_GPU}
export TRAIN_CUDA_DEVICES=${TRAIN_GPU}
...
# MIMIC variants
MIMIC_VARIANTS=("small_continued" "large_continued")
...
echo "╔══════════════════════════════════════════════════════════════════════╗"
echo "║  MEGA PIPELINE 2 — MIMIC (small + large)                          ║"
echo "║  Train GPU: ${TRAIN_GPU} (Ada 48GB)                                       ║"
echo "║  Gen GPUs:  ${GEN_GPUS} (Blackwell 96GB + Ada 48GB, TP=${GEN_TP})              ║"
...
for VARIANT in "${MIMIC_VARIANTS[@]}"; do
    for PCT in "${PERCENTAGES[@]}"; do
        ...
        echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
        CUDA_VISIBLE_DEVICES=${TRAIN_GPU} BATCH_SIZE=8 bash scripts/finetuning_mimic.sh ${VARIANT} ${PCT}
...
echo ""
echo "██████████████████████████████████████████████████████████████████████"
echo "█  PHASE 3/3 — TEXT GENERATION (vLLM, TP=${GEN_TP}, GPUs: ${GEN_GPUS})          █"
echo "██████████████████████████████████████████████████████████████████████"
echo ""
